"""
SW403 P2 RAG System - Advanced AST-based Implementation
...
@click.group()
def cli():
    """SW403 P2 Advanced AST-based RAG System CLI."""
    pass


@cli.command()
@click.option("--host", default="0.0.0.0", help="Host to bind to")
@click.option("--port", default=8002, help="Port to bind to (P2 default: 8002)")
@click.option("--reload", is_flag=True, help="Enable auto-reload for development")
def serve(host: str, port: int, reload: bool):
    """Start the FastAPI server for P2 AST-based system."""
    print("Starting P2 AST-based System...")
...
def ingest(files: List[str], recreate: bool):
    """Ingest Python files using P2 AST-based chunking."""
...
async def _ingest(files: List[str], recreate: bool):
    ...
    print("P2 AST-based: Starting ingestion...")
    print(f"Processing {len(files)} files")
    
    # Initialize components
    initialize_model()
    vector_store = create_vector_store(collection_name="functions_p2")
    ...
    if all_chunks:
        ...
        if success:
            print(f"P2 Ingestion complete! Total functions: {len(all_chunks)}")
...
def search(query: str, top_k: int):
    """Search for functions using P2 AST-based system."""
...
@cli.command()
def status():
    """Show system status for P2 AST-based system."""
    asyncio.run(_status())


async def _status():
    """Async implementation of status command."""
    print("P2 AST-based Status")
    print("=" * 20)
...
@cli.command()
def test():
    """Run comprehensive P2 AST-based test."""
    asyncio.run(_test())
...
async def _test():
    ...
    try:
        ...
        print("3. Testing P2 AST chunking capability...")
        chunker = create_chunker()
...
class TestClass:
    def method(self, param: Optional[Dict[str, Any]]):
        ...
        pass
'''
        test_file = Path("test_p2_temp.py")
